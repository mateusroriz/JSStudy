...
export class ListaAnimaisResolver implements Resolve<Animais> { //objetivo desse servico eh carregar algo antes da rota ser resolvida
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Animais> {
    return this.usuarioService.retornaUsuario().pipe(
      ...
      take(1) //depois que o switchmap for finalizado finalizar ele
    )
  }
}
