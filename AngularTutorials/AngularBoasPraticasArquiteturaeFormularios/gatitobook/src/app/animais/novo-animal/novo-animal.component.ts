...
export class NovoAnimalComponent implements OnInit {
      ...
      .subscribe(
        (event: HttpEvent<any>) => {
          if (event.type === HttpEventType.UploadProgress) {
            ...
            this.percentualConcluido = Math.round(1000 * (event.loaded / total));
          }
        },
  ...
  gravaArquivo(arquivo: any): void {
    const [file] = arquivo?.files; //descructuring colocando o arquivo na primeira posicao do array
    this.file = file;
    const reader = new FileReader();
    reader.onload = (event: any) => (this.preview = event.target.result);
    reader.readAsDataURL(file); //fazendo o preview da foto
  }
}
